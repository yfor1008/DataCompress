...
class LZSS:
    ...
    def __init__(self, window_size=20, lookahead_buffer_size=4):
        ...
        self.threshold = 2 # 匹配长度阈值, 只有>=阈值时, 才进行压缩
    ...
    def encode(self, data):
        '''
        ### Docs: 编码
            - 对于不同的匹配长度length, 有2种输出
            - length>=threshold 时, 输出(0,p,l)
            - 否则, 输出(1,c)
        ### Args:
            - data: string, 待处理数据
        ### Returns:
            - compressed: list, 压缩后数据, [(0,p,l), (1,c), ...]
        '''
